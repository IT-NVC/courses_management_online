...
export class UserController {
  ...
  @Get('/getUser/:id')
  async getCompanyById(@Res() res: Response, @Param('id') id: string) {
  ...
  //api update user with email and user name
    ...
    async updateUser(
      ...
      const user: any = await this.userService.updateUser(userRequest._id, {username: updateUserDto.username||userRequest.username, email: updateUserDto.email||userRequest.email});
      return res.status(HttpStatus.OK).send(user);
    }
    ...
    async registerCourse(
      ...
      const userRequest = req.user
      const createEnrollment = await this.userService.registerEnrollment(userRequest._id,body.courseId)
    ...
    async cancelCourse(
      ...
      @Req() req: any,
        ...
        const userRequest = req.user
        const cancelEnrollment = await this.userService.cancelEnrollment(userRequest._id,body.courseId)
     ...
     async getListCourseRegister(
       ...
       @Query('page') page: number = 1, @Query('limit') limit: number = 10
         ...
         const userRequest = req.user
         const cancelEnrollment = await this.userService.getListCourseRegister(userRequest._id,page,limit)
         return res.status(HttpStatus.OK).send(cancelEnrollment);
     }
}
